...
func (ss *sshSession) launchProcess() error {
	// cmd stays local (not ss.cmd): launchProcess wraps it in a cmdExecutor
	// and assigns ss.executor, so our sshSession has no cmd field.
	cmd, forwardedEnv, err := ss.newIncubatorCommand(ss.logf)
	if err != nil {
		return err
	}
