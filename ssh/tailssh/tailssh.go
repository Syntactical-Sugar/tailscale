...
func (ss *sshSession) killProcessOnContextDone() {
	...
	ss.exitOnce.Do(func() {
		...
		// Upstream signals SIGHUP directly on ss.cmd.Process for POSIX
		// terminal-disconnect semantics (OpenSSH gets it implicitly via
		// PTY-master close, session.c:2246). We route through the executor
		// because the Windows delegation backend has no *os.Process to
		// signal - it tears down the proxied OpenSSH session instead.
		ss.executor.Kill()
	})
}
...
func (ss *sshSession) run() {
	...
	if err != nil {
		...
		if errors.Is(err, context.Canceled) {
			...
			// 127 = "command not found" per POSIX 2018 sh ┬º2.8.2
	...
	wg.Go(func() {
		...
		if _, err := io.Copy(rec.writer("o", ss), rdStdout); err != nil && !errors.Is(err, io.EOF) {
			// A PTY read fails once the child exits and the master end
			// closes. That is expected teardown, not a fault, and the
			// errno differs per platform (see errno_windows.go).
			if !(processDone.Load() && isPtyClosedErr(err)) {
	...
	if rdStderr != nil {
		wg.Go(func() {
			...
			if _, err := io.Copy(ss.Stderr(), rdStderr); err != nil {
	...
	if err == nil {
		...
		// Deliberately an interface assertion rather than upstream's
		// *exec.ExitError: the Windows delegation executor does not run an
		// exec.Cmd, so it cannot return that concrete type. *exec.ExitError
		// satisfies this interface, so Unix behaviour is unchanged.
	...
	// CHANNEL_CLOSE (deferred ss.Close). RFC 4254 ┬º6.10. CloseWrite
